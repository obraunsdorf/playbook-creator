...
#include "mod.rs.h"  // Rust bridge via cxx (generated by cxx-build)
...
void PBCPlayView::paintPlayName() {
    ...
    const QString name =
            _renderPlay->codeName != "" ?
            QString::fromStdString(_renderPlay->codeName) :
            QString::fromStdString(_renderPlay->name);
...
void PBCPlayView::repaint() {
    ...
    if (_renderPlay) {
        for (PBCPlayerSP playerSP : *(_renderPlay->formation)) {
...
void PBCPlayView::createNewPlay(const std::string &name,
    ...
    PBCFormationSP formationSP =
        PBCController::getInstance()->getPlaybook()->getFormation(formationName);
    _renderPlay = PBCPlayRenderData{name, codeName, "", formationSP};
    setActivePlay();
    repaint();
}
...
/**
 * @brief Builds a Rust bridge snapshot from the current render data.
 */
static pbc2rust::BridgePath encodePathBridge(PBCPathSP path) {
    pbc2rust::BridgePath bp;
    bp.x = path->endpoint().get<0>();
    bp.y = path->endpoint().get<1>();
    bp.has_control = path->bezierControlPoint().get<0>() != DUMMY_POINT.get<0>();
    bp.cx = bp.has_control ? path->bezierControlPoint().get<0>() : 0.0;
    bp.cy = bp.has_control ? path->bezierControlPoint().get<1>() : 0.0;
    return bp;
}

static pbc2rust::BridgeRoute encodeRouteBridge(PBCRouteSP route) {
    pbc2rust::BridgeRoute br;
    if (route == NULL) {
        return br;  // empty name signals "no route"
    }
    br.name = rust::String(route->name().c_str());
    br.code_name = rust::String(route->codeName().c_str());
    for (const PBCPathSP& path : route->paths()) {
        br.paths.push_back(encodePathBridge(path));
    }
    return br;
}

pbc2rust::BridgePlaySnapshot buildBridgeSnapshot(const PBCPlayRenderData& renderPlay) {
    pbcAssert(renderPlay.formation != NULL);
    pbc2rust::BridgePlaySnapshot snapshot;
    snapshot.name = rust::String(renderPlay.name.c_str());
    snapshot.code_name = rust::String(renderPlay.codeName.c_str());
    snapshot.comment = rust::String(renderPlay.comment.c_str());

    for (PBCPlayerSP playerSP : *(renderPlay.formation)) {
        pbc2rust::BridgePlayer bp;
        bp.name = rust::String(playerSP->name().c_str());
        bp.nr = playerSP->nr();
        bp.r = playerSP->color().r();
        bp.g = playerSP->color().g();
        bp.b = playerSP->color().b();
        bp.role_full_name = rust::String(playerSP->role().fullName.c_str());
        bp.pos_x = playerSP->pos().get<0>();
        bp.pos_y = playerSP->pos().get<1>();

        bp.route = encodeRouteBridge(playerSP->route());
        bp.alt_route1 = encodeRouteBridge(playerSP->alternativeRoute(1));
        bp.alt_route2 = encodeRouteBridge(playerSP->alternativeRoute(2));

        for (PBCRouteSP optRoute : playerSP->optionRoutes()) {
            bp.option_routes.push_back(encodeRouteBridge(optRoute));
        }

        if (playerSP->motion() != NULL) {
            for (const PBCPathSP& path : playerSP->motion()->paths()) {
                bp.motion_paths.push_back(encodePathBridge(path));
            }
        }

        snapshot.players.push_back(std::move(bp));
    }
    return snapshot;
}
...
void PBCPlayView::renameAndSavePlay(const std::string& name,
    ...
    // Primary write: C++ playbook (needed by PBCStorage serialization)
    PBCController::getInstance()->getPlaybook()->deletePlay(_renderPlay->name);
    _renderPlay->name = name;
    _renderPlay->codeName = codeName;
    PBCPlaySP playToSave(new PBCPlay(_renderPlay->name, _renderPlay->codeName, "", _renderPlay->comment));
    playToSave->setFormation(_renderPlay->formation);
    PBCController::getInstance()->getPlaybook()->addPlay(playToSave, true);

    // Shadow write: Rust controller (dual-write, Option B)
    pbc2rust::pbc_save_play_snapshot(buildBridgeSnapshot(*_renderPlay));

    showPlay(_renderPlay->name);
}
...
void PBCPlayView::showPlay(const std::string& name) {
    ...
    pbcAssert(play != NULL);
    // Deep copy via PBCPlay copy ctor: creates a new formation with all routes/motions
...
void PBCPlayView::saveFormation(const std::string &formationName) {
    if (!_renderPlay)
        return;
    if (formationName != "") {
        _renderPlay->formation->setName(formationName);
    }
    PBCController::getInstance()->getPlaybook()->addFormation(_renderPlay->formation, true);
}
...
void PBCPlayView::editCategories() {
    if (!_renderPlay)
        return;
    savePlay();
    PBCPlaySP originalPlay = PBCController::getInstance()->getPlaybook()->getPlay(_renderPlay->name);  // NOLINT
...
void PBCPlayView::setActivePlay() {
    MainDialog* mainDialog = dynamic_cast<MainDialog*>(this->parent());
    if (mainDialog != NULL && _renderPlay) {
        mainDialog->fillPlayInfoDock(
            _renderPlay->name, _renderPlay->codeName, _renderPlay->comment);
    }
}
