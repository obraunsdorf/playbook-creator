...
void PBCStorage::checkVersion(const std::string& version) {
    ...
    if(result < 0) {
        throw PBCStorageException(
            "Cannot load playbook because it's created by a newer version of Playbook-Creator. "
            "Please download the latest version of Playbook-Creator!"); // NOLINT
    }
    assert(result >= 0);
...
void PBCStorage::generateAndSetKey(const std::string& password) {
    ...
    Botan::OctetString key =
        pbkdf->derive_key(_KEY_SIZE, password, &salt[0], salt.size(), _PBKDF_ITERATIONS);
 ...
 */
void PBCStorage::setCryptoKey(Botan::OctetString key, Botan::SecureVector<Botan::byte> salt) {
 ...
 */
void PBCStorage::encrypt(const std::string& input, std::ofstream& outFile) {
    ...
    Botan::Pipe encryptor(Botan::get_cipher(_CIPHER, *_keySP, Botan::ENCRYPTION),
 ...
 */
void PBCStorage::decrypt(const std::string& password, std::ostream& ostream,
                         std::ifstream& inFile) {
    ...
    Botan::Pipe decryptor(Botan::get_cipher(_CIPHER, key, Botan::DECRYPTION),
                          new Botan::Fork(new Botan::Hash_Filter(_HASH, _HASH_SIZE),
 ...
 * @param fileName
 */
void PBCStorage::savePlaybook(const std::string& password, const std::string& fileName) {
...
void PBCStorage::automaticSavePlaybook() {
    if(_keySP != NULL && _saltSP != NULL) {
        ...
        throw PBCStorageException("Cannot save playbook automatically whithout key. The playbook "
                                  "must be saved manually first"); // NOLINT
    }
}
...
void PBCStorage::writeToCurrentPlaybookFile() {
    ...
    std::string extension =
        _currentPlaybookFileName.substr(_currentPlaybookFileName.size() - 4); // NOLINT
    ...
    std::ofstream ofstream(_currentPlaybookFileName, std::ios_base::out | std::ios_base::binary);
 ...
 */
void PBCStorage::loadPlaybook(const std::string& password, const std::string& fileName) {
    ...
    try {
        decrypt(password, ostream, ifstream);
    } catch(std::exception& e) {
...
void PBCStorage::exportAsPDF(const std::string& fileName,
                             ...
                             const unsigned int paperWidth, const unsigned int paperHeight,
                             const unsigned int columns, const unsigned int rows,
                             const unsigned int marginLeft, const unsigned int marginRight,
                             const unsigned int marginTop, const unsigned int marginBottom) {
    std::string extension = fileName.substr(fileName.size() - 4);
    ...
    printer.setPageMargins(marginLeft, marginTop, marginRight, marginBottom, QPrinter::Millimeter);
    assert(columns > 0);
    assert(rows > 0);
    ...
    printer.getPageMargins(pixelMarginLeftSP.get(), pixelMarginTopSP.get(),
                           pixelMarginRightSP.get(), pixelMarginBottomtSP.get(),
    ...
    QRectF borderRect(10, 10, printer.paperRect().width() - 10, printer.paperRect().height() - 10);
    if(paintBorder == true) {
